...
//		 항상 자식쪽 메서드가 호출된다. 메서드 오버라이딩은
//		 자식쪽에 정의한 메서드가 부모쪽에 정의된 메서드를 
//		 가리게 된다.
...
//		child.walk();
//		//자식쪽에 정의된 메서드로 오버로딩으로 정의 되었다. 
//		child.walk(25);
//		
//		/*정적메서드는 클래스명을 통해서 호출한다. static의 기본규칙을 따르고
//		 오버라이딩의 대상이 될수 없다. */
...
//		
//		/*오버라이딩 처리된 메서드 이므로 참조변수에 상관없이
//		 자식쪽 메서드가 호출된다.
//		 자식객체가 생성될때 부모쪽에 메서드가 가려지게 되므로
//		 참조변수에 영향을 전혀 받지 않는다.*/
//		parent.exercise();
//		parent.sleep();
//		parent.printParent();
//		
//		
//		/*부모클래스의 참조변수로 자식 인스턴스를 참조하면 부모의 영역까지만 접근할 수 있으므로
//		 자식 영역에 정의된 메서드는 호출할 수 없다. */
//		parent.study(); // 에러발생
//		parent.walk();
//		parent.walk(20); //에러발생
//		
//		//정적 메서드는 오버라이딩 되지 않는다. 클래스명으로 호출한다.
