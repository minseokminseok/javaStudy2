...
/*
 ...
 : 메서드 오버로딩을 대체할수 있는 문법으로 하나의 메서드로
 여러가지 호출형식을 대체할 수 있다.
 형식]
 	메서드명(자료형 ... 매개변수명){
 		실행문장;
...
//	}
	...
	/*...을 통해 매개변수 param이 2개이상의 인수를
	 받을수 있는것을 명시한다. 또한 메서드 내에서는
	 배열로 반환되어 매개변수명.length와 같이 
	 사용할 수 있다.*/
	 static int getTotal(int ... param) {
	  int total = 0;
	...
	public static void main(String[] args) {
		...
		System.out.println("getTotal(args1)호출:" +
				getTotal(10));
		System.out.println("getTotal(args1, arge2)호출:"+
				getTotal(10,20));
		...
		/*매개변수의 타입이 double이 포함되어 있으므로 
		 호출할 수 없다. 만약 아래와 같이 실수를 모두 처리하려면
