...
/*instanceof 연산자
 ...
 -객체간의 형변환이 되려면 반드시 두 클래스간에 상속관계가 있어야한다.
 부모타입의 참조변수로 자식 인스턴스를 참조할 수 있으므로
 매개변수를 부모타입으로 만들면 자식 인스턴스를 인수로 받을 수 있다.
 -매개변수로 전달되는 인수는 부모타입으로 자동형변환(업 캐스팅)되므로
...
public class E09InstanceOf {
	...
	/*매서드 정의시 부모타입의 매개변수로 설정하면 
	 자식타입의 인스턴스를 한꺼번에 받을 수 있다. 
	 -Box타입으로 인수를 받게되면 자동형변환(업캐스팅)되므로 
	 자식클래스의 맴버메서드를 호출할 수 없게된다. 
	 이때 자식의 맴버메서드를 호출하기 위해 강제형변환(다운캐스팅)이 
	 필요하다. 
	 instanceof 연산자를 통해 자식타입으로 형변환이 가능한지 
	 판단 한 후 true를 반환하면 다운캐스팅을 진행한다. 
	 */
	...
	/*만약 부모타입의 매개변수로 자식타입의 인수를 받을 수 없다면
	 아래와 같이 각각의 타입에 따른 모든 메서들르 정의해야 한다.
	 
	static void WrapBox(Box b) {}
	static void WrapBox(PaperBox b) {}
	static void WrapBox(GoldPaperBox b) {}
	
	상속관계가 더 깊어진다면 메서드는 점점 늘어나게 되어 코드는
	더욱 더 복잡해지게 될것이다. 따라서 부모로 자식을 참조할 수 있는것은
	개발자에게 편리함을 가져다준다. 
	 */
	...
	public static void main(String[] args) {
		...
		/*String 인스턴스는 Box와는 아무런 관계가 없으므로 
		 인수로 전달할 수는 없다. 즉 상속관계가 성립하지 않는다. */
		String str = new String("나는 누구?");
		//wrapBox(str); //에러발생 상속관계가 없으니까 
