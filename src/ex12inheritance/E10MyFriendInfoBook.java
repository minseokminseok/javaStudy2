...
/*친구를 표현한 최상위 클래스로 해당 프로그램에서는 Friend클래스로
 ...
 정의된 클래스이다. 이와 같이 데이터를 저장하기 위한 용도의 클래스를 가리켜 VO(Value Object)
 라고 한다. 즉 값만 가진 객체라는 뜻이다. */

class Friend{
	//맴버변수 : 이름, 전화번호, 주소 기본정보 3가지를 저장.
	String name;
	String phone;
	String addr;
	//생성자 : 맴버변수 초기화.
...
class HighFriend extends Friend {
	//확장한 맴버변수 : 별명
	String nickname;
	//생성자 : 부모의 생성자를 먼저 호출한 후 맴버변수 초기화.
	...
	/*오버라이딩1
	 ...
	 super를 통해 호출하고, 자식에서 확장한 변수를 추가해서 
	 출력한다. */
	@Override
	public void showAllData() {
	...
	/*오버라이딩2 : 자식 클래스에서 오버라이딩하여 재정의한 메서드
	 로 고딩친구의 간략한 정보를 출력한다. 부모클래스에서는
	 실행부가 없는 상태로 정의되었다. */
	@Override
	public void showBasicInfo() {
	...
	class UnivFriend extends Friend {
		...
		/*오버라이딩1,2 역시 HighFriend클래스와 동일한 패턴으로 
		 정의됨*/
		@Override
		public void showAllData() {
...
public class E10MyFriendInfoBook {
	...
	/*메뉴를 출력하는 용도의 메서드로 매개변수, 반환타입이 없는 형태로 정의한다.
	 main메서드에서 직접 호출하기 위해 static으로 선언되어있다.*/
	...
	/*무한루프 조건으로 사용자가 원할때 종료할 수 있는 구조를 만들어준다.
	 break문은 반복문을 탈출시키는 기능이 있으므로 이와같은 무한 루프에서 자주 
	 사용한다.
	 main메서드에서는 특별한 기능은 구현하지 않고 프로그램 전반의 흐름에 대해서만
	 담당한다.*/
