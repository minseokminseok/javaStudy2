...
abstract class AbsClass1{
	...
	//int 형으로 표현할 수 있는 최대값을 정적상수로 선언
...
/*추상 클래스2 : 추상메서드를 포함한 클래스. 추상 메서드를 맴버로
 포함한 클래스는 반드시 abstract로 선언해야 한다. 
 그렇지 않으면 에러가 발생한다.*/
...
/*위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속받게
 ...
 해당 클래스로도 인스턴스를 생성할 수 없게된다. 따라서 
 객체생성을 위해서는 부모의 추상메서드를 자식쪽에서 오버라이딩 하면
 된다. */
class AbsClassChild2 extends AbsClass2{
	/*오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의
	 하는것이므로 해당 클래스는 추상 메서드를 포함하지 않은 상태가된다.
	 따라서 인스턴스 생성이 가능해진다. 아래 오버라이딩한 메서드를 삭제하면
	 에러가 발생한다.*/
	@Override
...
public class E01AbstractMain {
	...
	//	AbsClass1 absClass1 = new AbsClass1();
		...
		/*추상클래스는 인스턴스 생성은 할수 없지만 참조변수로는 사용할 수 있다.*/
		AbsClass2 absClass2 = new AbsClassChild2();
		//부모쪽의 맴버메서드는 호출할 수 있다.
		...
		/*추상클래스내에 선언된 정적메서드는 오버라이딩의 대상이 될 수 없으므로
		 static의 기본규칙을 따른다. 별도의 생성절차 없이 클래스명으로
		 바로 접근할 수 있다. */
