...
class PersonalInfoDTO{
	...
	/*getter / setter :DTO객체에서 private으로 선언된 맴버변수 
	의 값을 설정하거나 반환 받을때 사용한다. */
...
/*제공된 추상클래스를 상속하여 한사람의 정보를 저장하는 기능과 검색의 기능을
 구현한 클래스*/
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	//정보저장용 객체배열과 인덱스 카운트를 위한 맴버변수 선언
	...
	/* 상속을 통해 추상메서드를 포함하게 되므로 반드시 아래와같이
	 오버라이딩 해야한다. 이는 필수사항이며 하지 않을 경우 에러가 발생한다.*/
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i=0; i<numOfPerInfo; i++) {
			...
			if(juminNum.compareTo(personalArr[i].getJuminNum())==0) {
				...
				/*DTO객체의 맴버변수는 private으로 선언되었으므로
				 외부에서는 접근이 불가능하다. 따라서 getter을 통해 
				 접근해야한다.*/
			}
