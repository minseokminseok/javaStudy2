...
/*
 ...
 구현받은 하위클래스에선느 반드시 추상메서드를 오버라이딩 해야한다.
 맴버로는 추상메서드와 상수만 선언할 수 있다.
 	메서드 : public abstract
...
class MyClass extends SimpleClass implements MyInterface1, MyInterface2{
	...
	/*인터페이스는 동일한 이름의 추상메서드가 존재하는 경우
	 하나만 오버라이딩 하면 된다. 오버라이딩은 항상
	 자식쪽의 메서드가 호출되므로 아무런 문제도 발생되지 않는다.
	 */
	@Override
	...
	/*SimpleClass 클래스의 맴버메서드는 필수사항이 아니므로
...
public class E03InterfaceMain {
	...
	public static void main(String[] args) {
		...
		my1.absFunc1();
		my1.absFunc2();
		//자식쪽 맴버에 접근하려면 다운캐스팅 후 호출해야한다.
		((SimpleClass)my1).mySimple();
		/*MyClass의 부모는 모두3개이므로 아래는 모두
		 참조가 가능하다. 1*/
