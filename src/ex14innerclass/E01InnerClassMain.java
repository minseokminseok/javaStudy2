...
/*
 ...
 -외부 클래스의 모든 맴버(정적 혹은 인스턴스형)를 사용할 수 있다.
 정적 맴버를 가질 수 없다. 단 상수는 가질 수 있다.
...
class Unit{
	...
	//인스턴스형 맴버변수 : 사용시 new를 통해 인스턴스를 생성한다.
	Marine marine;
	Medic medic;
	...
	//내부클래스정의(내부 맴버 클래스)
	class Marine{
		//내부 클래스도 일반적인 클래스와 동일하게 맴버를 정의할수 있다.
		int attackPower;
		/*내부클래스에서는 외부클래스의 모든 맴버에 접근할 수 있다.
...
public class E01InnerClassMain {
	...
	public static void main(String[] args) {
		//내부 접근 위해 외부먼저 접근해야된다.
		//외부 클래스의 인스턴스를 먼저 생성한다.
		Unit unit = new Unit("커멘트센터", 1000);
		System.out.println(unit.infoStr());
		//Marine은 클래수 내부에 있어서 바로 만들수는 없다.
		/*외부클래스가 아닌 다른 클래스에서는 내부 맴버 
		 클래스가 보이지 않으므로 직접 인스턴스를 생성할 수 없다.*/
		// Marine m = new Marine();
		
		/*외부클래스의 인스턴스를 먼저 생성한 후 내부클래스의 인스턴스를
		 생성해야한다. Marine은 Unit클래스의 생성자에서 이미 
		 인스턴스 생성이 되어 있으므로 아래와 같이 사용할 수 있다.*/
		Unit.Marine marine = new Unit("마린",50).marine;
		marine.attack();
		/*Medic의 경우 Unit의 생성자에서 인스턴스 생성이 되지 않았으므로 
		 아래와 같이 new를 통해 생성해야 한다. 이때에도 Unit 인스턴스를
		 통해 생성해야 하므로 new를 두번 사용해서 인스턴스를 생성한다.*/
