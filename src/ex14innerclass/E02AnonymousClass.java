...
/*
 ...
 형식]
 	...
 	부모클래스를 상속받아 이름없는 자식 클래스를 생성하는 개념으로
...
class Singer{
	...
	/*Object클래스에 정의된 메서드로 객체가 가진 값을 문자열로 
	 반환해주는 메서드이다. Java에서 정의하는 모든 클래스는
	 Object 클래스를 상속하므로 이와같이 오버라이딩이 가능하다.*/
	@Override
...
/*인터페이스 정의 : 기능이 없는 추상메서드만 맴버메서드로 정의할수
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*GirlGroup클래스를 상속받은 익명클래스 정의
		 ...
		 생성해서 그 주소값을 할당한다.
		 개념적으로 new GirlGroup(){};
		 	=> 익명 extends GirlGroup 과 같다 동일하다.
		...
		Singer s3 = new GirlGroup("아이린", "레드벨벳") {
			...
			@Override
			public String toString() {
				return super.toString()+", 소속사SM";
			}
		};
		/*부모의 참조변수로 자식을 참조할 수 있지만, 접근범위가 
		 부모까지로 제한된다. 따라서 아래와 같이 자식에서 확장한 
		 맴버변수는 접근할 수 없어 에러가 발생한다.
		 접근하고 싶다면 다운캐스팅(강제형변환)을 해야하지만
		 익명클래스는 이름이 없기때문에 다운캐스팅 자체가 불가능하다.
		 따라서 익명클래스에서는 맴버의 확장은 거의 사용되지않는다.
		 대부분 오버라이딩을 목적으로 확용된다. */
		//익명함수는 다운캐스팅이 안된다.
