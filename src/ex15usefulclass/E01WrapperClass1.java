...
/*
 Wrapper : 기본자료형의 데이터를 인스턴스화 할때 사용하는 클래스로
 	Boxing 혹은 Unboxing 을 통해 기본자료형의 데이터를 인스턴스화 
...
public class E01WrapperClass1 {
	...
	public static void main(String[] args) {
		...
		/*JDK1.4 이전의 코딩형태 
		 ...
		 진행할때 언박싱을 위해 intValue()와 같이 
		 메서드를 호출해야한다.*/
		
		//현재는 이와같은 생성자는 Deprecated 되었다.
		...
		/* JDK1.5 이후 버전에서의 코딩형태
		 오토박싱 혹은 오토언박싱이 지원되므로 기본 자료형 변수를
		 선언하는것과 동일하게 코딩할 수 있다.*/
		//별도의 형변환이나 처리 없이 기본자료형처럼 선언한다.
		Integer numObj1 = 100;
		Integer numObj2 = 200;
		int num3 = numObj1;
		//객체이지만 정수처럼 연산이 가능하다.
