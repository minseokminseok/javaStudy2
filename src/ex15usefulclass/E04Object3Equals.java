...
class MyClass{
	...
	/*MyClass 인스턴스간의 내용 비교를 위해 equals()를 
	 오버라이딩 하여 재정의 한다. 인스턴스가 가진 맴버변수의 값에 대한
	 비교를 통해 동일 여부를 판단한다. equals()메서드의 매개변수 
	 타입은 모든 인스턴스를 대상으로 하므로 Object형으로 
	 정의되어 있다. */
	@Override
	public boolean equals(Object obj) {
		/*매개변수로 전달된 인스턴스를 Object로 받으면 
		 업캐스팅이 되므로 다운캐스팅 이후에 비교를 해야된다.
		 이때 전달된 인스턴스가 MyClass형이 아니라면
		 비교의 대상이 될 수 없으므로 상속관계 확인을 위해
		 instanceof 연산자를 사용한다.*/
	
		if(obj instanceof MyClass) {
			/*매개변수가 MyClass타입이 맞다면 
			 비교를 위해 다운캐스팅 한다. 그렇지 않으면
			 부모타입으로는 자식 맴버에 접근할 수 없으므로
			 맴버변수 data에 대한 비교가 불가능해진다.*/
		
			MyClass mc = (MyClass)obj;
			//맴버변수 값을 비교하여
			if(mc.data == this.data) {
				//값이 동일하면 true를 반환한다.
				System.out.println("MyClass-맴버동일함");
				return true;
			}
			else {
				//다르면 false를 반환한다. 
				System.out.println("MyClass-맴버다름");
				return false;
			}
		}
		else {
			/*매개변수로 전달된 인스턴스가 MyClass 타입이 */
