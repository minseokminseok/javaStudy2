...
public class E05Date {
	...
	public static String myDateFormat(Calendar c, String formatStr) {
		/*getTime() : 서버의 표준시인 1970-01-01 부터 
		 현재까지의 시간을 초 단위로 반환해준다. 이를 타임스태프라고
		 한다.*/
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		...
		/*Calendar 클래스는 대표적인 싱글톤 객체이므로 
		 new로 인스턴스를 생성할 수 없고 유틸리티 메서드를 통해
		 참조값을 얻어온다. */
		...
		/*현재 날짜를 기준으로 전/ 후 글 구하기 위해 
		 add()메서드를 사용한다. 만약 음수를 주면 과거의 날짜를
		 구할수 있다. */
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			/*getTime()으로 얻어오는 시간은 13자리의 정수값인데 
			 ...
			 제거하면 순수한 초가 반환된다. 또한 하루는 60*60*24즉
