...
public class Ex05Finally {
	...
	static void tryCatchFinally() {
		...
		int age = -1;
		try {
			// 10살과 같이 입력하면 예외가 발생한다.
			age = scan.nextInt(); //
			System.out.println("당신은 5년후 " +(age+5) +"살 입니다.");
			//앞에서 정상적인 값을 입력하더라도 return 이 실행된다.
			//일반적으로 return은 메서드의 종료의 의미하지만 finally절이
	...
	public static void main(String[] args) {
		...
		/* NumberFormatException 과 같은 예외는 발생된 지점에서
		 예외던지기를 하는것은 가능하나, 반드시 예외처리를 해야만 프로그램이 종료되지 않는다.
		 즉 예외객체에 따라 무시할 수 있는것도 있지만 이처럼 반드시 예외처리를 해야하는것도 
		 존재한다. 만약 main메서드에 throws NumberFormatException과 같이
		 선언하면 프로그램은 비정상 종료된다. */
		try {
			runtime();
		}
