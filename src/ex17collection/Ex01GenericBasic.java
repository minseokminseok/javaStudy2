...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		/*아래와 같이 Orange가 아닌 객체를 저장하면 
		 즉시 컴파일에러가 발생하여 실행할 수 없는 코드가 된다.
		 즉 자료형에 안전하다고 할수 있다. */
//		oBox1.store("당도가 20인 오렌지"); 신택스에러 
		...
		/*Object 기반으로 생성된 FruitBox는 구현에는 편리하나
		 ...
		 훨씬 더 어렵다*/
		//Orange 객체를 저장후 꺼내는건 문제없이 실행된다.
		FruitBox fBox1 = new FruitBox(new Orange(20));
		Orange orange3 = (Orange)fBox1.pullOut();
		orange3.showInfo();
		
		/*하지만 오렌지 같은 String을 저장하고, 이를 꺼내서 
		 다운캐스팅 하는 부분에서는 예외가 발생된다. 즉
		 런타임 에러가 발생하게 되므로 실행해보기 전에는 오류를 
		 예측하기 쉽지않은 단점이 있다. .*/
		FruitBox fBox2 = new FruitBox("당도가30인 오렌지");
